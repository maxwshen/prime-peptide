...
def combine_dfs(exp_conds, stat_col):
  ...
  for cond in exp_conds:
    df = pd.read_csv(inp_dir + f'{cond}_control.csv', index_col = 0)
...
# Primary
##
def solve_mle_beta_parameters(target, stat_col, exp_subset):
  '''
...
def main(argv):
  ...
  solve_mle_beta_parameters(target, stat_col, exp_subset)
